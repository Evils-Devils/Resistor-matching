...
#deviation from mean in ppm for 44 resistors, averaged multiple measurements (sorted)
...
for subset in itertools.combinations(n, k):#try every combination of k from n
    if abs(numpy.mean(subset)-target) <= tolerance: #check if the combination is worth anything
        ...
        print (count) #separated because python...
        print (datetime.datetime.now().time(),"\n",numpy.mean(subset),"\n",subset,"\n") #prints timestamp, found value and items that make up the value
